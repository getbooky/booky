...
import { AddToLibraryButton, MonitorSwitch } from "@/components/AddToLibrary"
...
export function SeriesDetailView({ series, onBack, onOpenBook, onOpenAuthor }: {
  ...
  // behavior as the author page's per-book toggle. The switch itself asks
  // which library when there are several (MonitorSwitch) — toggleBook only
  // ever sees the direct cases.
        ...
        {shown.map(b => (
          <div key={`${b.id}-${b.libraryId}`} className="grid grid-cols-[34px_40px_1fr_auto] items-center gap-4 border-b border-linesoft px-1 py-2.5">
            ...
            <div className="flex items-center justify-end gap-2.5">
              ...
              <MonitorSwitch book={b} libraries={libraries} onToggle={toggleBook} onPick={monitorInto} />
            </div>
          </div>
        ))}
      </div>
